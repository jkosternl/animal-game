...
public class Animal {
    ...
    public void changeOrientation(final double delta) {
        ...
        if (orientation >= (Math.PI * 2d))
            orientation -= (Math.PI * 2d);
        if (orientation < 0)
            orientation += (Math.PI * 2d);

        if (orientation >= EAST && orientation < SOUTH)
            quadrant = 1;
        else if (orientation >= SOUTH && orientation < WEST)
            quadrant = 2;
        else if (orientation >= WEST && orientation < NORTH)
            quadrant = 3;
        else quadrant = 4;
    }
    ...
    public void log() {
        ...
        log.info("Snake {} is at position[{}, {}], orientation {}, quadrant {}, corrections: {}, steps: {}",
            number, x, y, orientation, quadrant, corrections, stepsAlive);
    }
